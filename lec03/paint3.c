...
int count_neighbor(int i, int j) {
  ...
  int c = 0;
  for (s = i-1; s <= i+1; s++) {
    if (s < 0 || WIDTH <= s) continue;
    for (t = j-1; t <= j+1; t++) {
      if (t < 0 || HEIGHT <= t) continue;
      if (s == i && t == j) continue;
      if (canvas[s][t] == '#') c++;
    }
  }
...
void detect_edge() {
  char next_canvas[WIDTH][HEIGHT];

  int i, j;
  for (i = 0; i < WIDTH; i++) {
    for (j = 0; j < HEIGHT; j++) {
      next_canvas[i][j] = canvas[i][j];
    }
  }

  for (i = 0; i < WIDTH; i++) {
    for (j = 0; j < HEIGHT; j++) {
      if (count_neighbor(i, j) == 8 && canvas[i][j] == '#') {
        next_canvas[i][j] = ' ';
      }
    }
  }
