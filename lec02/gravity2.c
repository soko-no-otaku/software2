...
void plot_stars(FILE *fp, const double t)
{
  ...
  printf("t = %5.1f", t);
  for (i = 0; i < nstars; i++)
    printf(", stars[%d].(x, y) = (%7.2f, %7.2f)", i, stars[i].x, stars[i].y);
  printf("\n");
}

double distance_stars(struct star *a, struct star *b) {
  return sqrt(pow(a->x - b->x, 2) + pow(a->y - b->y, 2));
}

int is_neighbor(struct star *a, struct star *b) {
  const double border = 5.0;
  return (distance_stars(a, b) < border)? 1 : 0;
}
...
void update_velocities(const double dt)
{
  ...
  for (i = 0; i < nstars; i++) {
    ...
    ax = 0; ay = 0;
    for (j = 0; j < nstars; j++) {
      if (!is_neighbor(&stars[i], &stars[j])) {
        if (stars[i].m > 0.0) {
          double dr = distance_stars(&stars[i], &stars[j]);
